...
public class DeAllocateButterOrderAction implements Action<ButterOrderStatusEnum, ButterOrderEventEnum> {
    ...
    public void execute(StateContext<ButterOrderStatusEnum, ButterOrderEventEnum> context) {
        ...
        log.info("Sending DeAllocateButterOrderRequestEvent to DE_ALLOCATE_ORDER_QUEUE for Butter Order : " + orderId);
        jmsTemplate.convertAndSend(JmsConfig.DE_ALLOCATE_ORDER_QUEUE,
                                   DeAllocateButterOrderRequestEvent.builder()
                                           .butterOrderDto(butterOrderMapper.butterOrderToDto(butterOrder)).build());
        log.info("Sent DeAllocateButterOrderRequestEvent to DE_ALLOCATE_ORDER_QUEUE for Butter Order : " + orderId);
